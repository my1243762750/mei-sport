...
const FootprintsIcon: React.FC<{ active: boolean; mirrored?: boolean }> = ({ active, mirrored }) => (
  <svg
    ...
    style={{
      color: active ? '#a78bfa' : 'rgba(255, 255, 255, 0.08)',
      transform: mirrored ? 'scaleX(-1)' : undefined,
    }}
  >
    <path d={FOOTPRINT_PATH} fill="currentColor" />
  </svg>
);

export const LeftRightFootGuide: React.FC<LeftRightFootGuideProps> = ({ isPlaying, beatTrigger }) => {
  const activeFoot = beatTrigger % 2 === 0 ? 'L' : 'R';
  const isLeftActive = isPlaying && activeFoot === 'L';
  const isRightActive = isPlaying && activeFoot === 'R';

  return (
    <div className="foot-guide-card" aria-label="左右脚节奏提示">
      <div className="foot-guide-row">
        <div className={`foot-guide-pad ${isLeftActive ? 'active' : ''} ${!isPlaying ? 'disabled-mode' : ''}`}>
          <FootprintsIcon active={isLeftActive} />
          <span className="foot-guide-label">左脚</span>
        </div>
        <div className={`foot-guide-pad ${isRightActive ? 'active' : ''} ${!isPlaying ? 'disabled-mode' : ''}`}>
          <FootprintsIcon active={isRightActive} mirrored />
          <span className="foot-guide-label">右脚</span>
        </div>
      </div>
    </div>
  );
